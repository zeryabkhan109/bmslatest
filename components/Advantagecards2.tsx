      ...
      <div className="flex flex-col md:w-1/2 w-full">
        {BENEFITS.map((benefit, i) => (
          <button
            key={i}
            onClick={() => setActive(i)}
            className={`text-left w-full md:p-6 p-4 rounded-2xl transition-all duration-300 cursor-pointer ${active === i ? "bg-white/10" : "bg-transparent hover:bg-white/5"
              }`}
          >
            <p
              className={`xl:text-2xl text-sm font-semibold leading-4.75 xl:leading-8.25 transition-colors duration-300 ${active === i ? "text-orange-1100" : "text-white/50"
                }`}
            >
              {benefit.title}
            </p>
            ...
            >
              <div className="overflow-hidden">
                <p
                  className={`xl:text-base font-normal text-xs leading-4 xl:leading-5.5 text-grey-1100 mt-4 transition-opacity duration-500 ${active === i ? "opacity-100" : "opacity-0"
                    }`}
                >
                  {benefit.description}
                </p>
                ...
                {i === BENEFITS.length - 1 && benefit.description2 && (
                  <p
                    className={`xl:text-base font-normal text-xs leading-4 xl:leading-5.5 text-grey-1100 italic mt-4 transition-opacity duration-500 ${active === i ? "opacity-100" : "opacity-0"
                      }`}
                  >
      ...
      <div className="md:w-1/2 w-full relative rounded-2xl overflow-hidden shadow-4xl">
        {BENEFITS.map((benefit, i) => (
          <img
            ...
            src={benefit.image}
            alt={benefit.title}
            className={`absolute inset-0 w-full h-full object-cover shadow-4xl transition-opacity duration-500 ${active === i ? "opacity-100" : "opacity-0"
              }`}
          />
        ))}
      </div>
