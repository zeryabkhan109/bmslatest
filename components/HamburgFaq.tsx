...
function FaqRow({
  ...
  `}
      >
        ...
        <span className="relative size-8 flex items-center justify-center">
          ...
          <img
            ...
            alt="minus icon"
            className={`absolute transition-all duration-300 ease-in-out ${open ? "opacity-100 scale-100" : "opacity-0 scale-75"
              }`}
          />
        </span>
      </button>
      ...
      >
        <div
          ref={contentRef}
          className="px-6 pt-4 md:pb-5"
        >
          <p className="md:text-base text-xs font-normal md:leading-5.5 leading-[140%] text-white/80">
            {item.answer}
          </p>
          <Link href={'#'} className="text-xs mt-4 font-light italic inline-flex items-center underline leading-[140%] text-white-1100">{item.more}</Link>
        </div>
      </div>
    </div>
  );
